...
def upgrade():
    ...
    sa.Column('id', sa.BigInteger(), nullable=False),
    sa.Column('state', Enum('new', 'fetching', 'processing', 'complete', 'error', 'removed', 'disabled', 'specialty_deferred', 'specialty_ready', 'not_set', name='dlstate_enum'), nullable=False),
    ...
    if settings.SQLALCHEMY_DATABASE_URI.startswith("sqlite://"):
        print("Sqlite backend selected. Not trying to do migration.")
    else:
        conn = op.get_bind()
        res = conn.execute('''
                SELECT
